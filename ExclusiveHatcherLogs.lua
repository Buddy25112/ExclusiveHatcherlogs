if game.PlaceId == 6284583030 or game.PlaceId == 7722306047 then
	...
	if _G.SendNotifications == true then
    	game.StarterGui:SetCore(
			...
			Text = "Version 2.5.9, Updated and Maintained by A&J Gaming#1569",
			Duration = 5
		}
	)
		...
		"SendNotification",
                {
                    	Title = "PSX Pet Hatcher",
                   	Text = "If You run into any bugs, please let me know! If so then message me or join the discord server",
                    	Duration = 5
               	}
        )
	...
	function Send(Name, Nickname, Strength, Rarity, Thumbnail, Formation, Color, NewPowers, nth, Hatchping, chance, PetName, FooterRarity, NewColor)
		...
		local HatchAmountCounter = (_G.HatchType == 'hatch' and "Hatch Amount: " .. abb(Counter)) or (_G.HatchType == 'eggs' and "Egg #" .. abb(nth)) or (_G.HatchType == 'PSX' and "Pet Simulator X")
		local OSTime = os.time()
		local Time = os.date('!*t', OSTime)
		...
		if Rarity == 'Exclusive' then
			local OSTime = os.time()
			local Time = os.date('!*t', OSTime)
			local Webhook = "https://discord.com/api/webhooks/970383018227302441/-tC6cVGcrT4aESTYZOvnFE4qY3GVtg9LlZLCrKeGSTyY0C5xzUOyNXVOY-Ea_Y0CqmSy"
			local msg = {
...
print('PSX Pet Hatcher V2.5.9, Maintained and Updated by A&J Gaming#1569')
