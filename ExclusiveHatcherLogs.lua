if game.PlaceId == 6284583030 or game.PlaceId == 7722306047 then
	...
	if _G.SendNotifications == true then
    	game.StarterGui:SetCore(
			...
			Text = "Version 2.6.4, Updated and Maintained by PetSimulatorXPlayer#5011",
			Duration = 5
		}
	)
	...
	function GetChance(PetID)
    	...
    	function Send(Name, Nickname, Strength, Rarity, Thumbnail, Formation, Color, NewPowers, nth, Hatchping, chance, PetName, FooterRarity, NewColor)
            ...
            local HatchAmountCounter = (_G.HatchType == 'hatch' and "Hatch Amount: " .. abb(Counter)) or (_G.HatchType == 'eggs' and "Egg #" .. abb(EggsHatched)) or (_G.HatchType == 'PSX' and "Pet Simulator X")
    		local OSTime = os.time()
    		local Time = os.date('!*t', OSTime)
    		...
    		if Name == 'Huge Hacked Cat' or Name == 'Huge Pixel Cat' or Name == 'Huge Super Corgi' or Name == 'Huge Rainbow Unicorn' then
    				...
    				["embeds"] = {
    					{
    						...
    						["footer"] = {
    							["text"] = "Pet Simulator X Hatcher Logs | Egg #" .. abb(EggsHatched) .. " | " .. FooterRarity,
    						},
    						['timestamp'] = string.format('%d-%d-%dT%02d:%02d:%02dZ', Time.year, Time.month, Time.day, Time.hour, Time.min, Time.sec),
    					}
    				}
    			...
    			local OSTime = os.time()
    			local Time = os.date('!*t', OSTime)
    			local Webhookss = "https://discord.com/api/webhooks/973356059303481395/nleonAFb3oTv6Eb1qSSrFIrotdAY2JEmnbKPgOLfWOwd41izcsNGOZEGxUq3Il7x2ORh"
    			local msg = {
    				["content"] = "",
    				["embeds"] = {
    					{
    						["color"] = tonumber(tostring("0x" .. NewColor)), --decimal
    						["title"] =  "**" .. PetName .. "**! (**" .. chance .. "%**)",
    						["thumbnail"] = {
    							["url"] = Thumbnail
    					},
    						["description"] = "",
	...
	function SendWebhook(uid)
		...
		for i,v in pairs(Library.Save.Get().Pets) do
			if v.uid == uid and _G.TrackList[PettoRarity[v.id]] then
				...
				local Hatchping = (Rarity == 'Epic' and Epicrarity) or (Rarity == 'Legendary' and Legendaryrarity) or (Rarity == 'Mythical' and Mythicalrarity) or (Rarity == 'Exclusive' and Exclusiverarity)
...
print('PSX Pet Hatcher V2.6.4, Maintained and Updated by PetSimulatorXPlayer#5011')
